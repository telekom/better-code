...
function GenerateCerts {
	...
	$splat = @{
		...
		Value    = ("-----BEGIN $privateKeyLabel-----`n" + [System.Convert]::ToBase64String($privateKeyFromCert.Key.Export([System.Security.Cryptography.CngKeyBlobFormat]::Pkcs8PrivateBlob), [System.Base64FormattingOptions]::InsertLineBreaks) + "`n-----END $privateKeyLabel-----");
		Encoding = "Ascii";
	}
	Set-Content @splat
	...
	$splat = @{
		...
		Value    = ("-----BEGIN $privateKeyLabel-----`n" + [System.Convert]::ToBase64String($clientprivateKeyFromCert.Key.Export([System.Security.Cryptography.CngKeyBlobFormat]::Pkcs8PrivateBlob), [System.Base64FormattingOptions]::InsertLineBreaks) + "`n-----END $privateKeyLabel-----");
		Encoding = "Ascii";
	}
	Set-Content  @splat
